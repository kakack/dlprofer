# !@file iteration_op_kernels_db.py
# 
...
class IterationOpKernelsDB(DatatablesDB):
    _sql_rows = None
    _sql_count = None
    ...
    def __init__(self, request):
        ...
        from_clause = " FROM view_iteration_op_kernels "
        where_clause = " WHERE aggr_id=? AND domain_name=? AND iter_value=? AND op_node_id=? "
        ...
        self._sql_args = [ str(aggr_id), domain_name, str(iter_value), str(op_id) ]
    ...
    def get_property_key(self):
        return 'iterationOpKernelsProp'

    #overrides(DatatablesDB)
    def get_search_constraint(self):
        constraint = ' AND ('
        constraint += "kernel_name LIKE ? "
